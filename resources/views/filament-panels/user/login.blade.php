    ...
    <div class="w-full max-w-md space-y-8">
        ...
        <div class="text-center">
            <div class="inline-flex items-center gap-3 rounded-full bg-white px-4 py-3 text-sm font-medium text-slate-700 shadow-sm dark:bg-slate-800 dark:text-slate-200 mb-6">
                <span class="inline-flex h-10 w-10 items-center justify-center rounded-full bg-blue-600 text-white">U</span>
                <span>E-DISPOS KPU PROVINSI BENGKULU - USER</span>
            </div>
            <h2 class="text-3xl font-semibold text-slate-950 dark:text-white">Masuk sebagai User</h2>
            <p class="mt-2 text-sm text-slate-600 dark:text-slate-400">Akses daftar surat dan disposisi Anda</p>
        </div>

        <!-- Login Form -->
        <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-lg p-8">
            <form method="POST" action="{{ route('filament.user.auth.login') }}" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <input
                        ...
                        required
                        value="{{ old('email') }}"
                        class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-slate-600 dark:bg-slate-800 dark:text-white"
                ...
                <div class="flex items-center">
                    <input
                        ...
                        name="remember"
                        type="checkbox"
                        class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-slate-300 rounded dark:border-slate-600"
                    >
